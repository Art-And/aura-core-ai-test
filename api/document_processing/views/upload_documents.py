...
from api.document_processing.serializers import UploadDocumentsSerializer
from api.document_processing.services import (
    EmbeddingsService,
...
class UploadDocumentsViewSet(mixins.CreateModelMixin, GenericViewSet):
    ...
    def add_require_data(self, blob_file):
    ...
    def create(self, request, *args, **kwargs):
        ...
        source_blob_name = GCPCloudStorageService.get_path_with_name(
            StorageConstants.Paths.TEMPORAL_DOCUMENT, file_name
        )
        destination_blob_name = GCPCloudStorageService.get_path_with_name(
            StorageConstants.Paths.PERMANENT_DOCUMENT, file_name
        )
        ...
        try:
            ...
            self.add_require_data(blob_file)
            return super().create(request, *args, **kwargs)

        except NotFound as e:
            return Response(
                {"error": f"The file {file_name} was not found.", "details": str(e)},
                status=status.HTTP_404_NOT_FOUND,
            )
        except Forbidden as e:
            return Response(
                {
                    "error": "Access denied to the bucket. Check your permissions.",
                    "details": str(e),
                },
